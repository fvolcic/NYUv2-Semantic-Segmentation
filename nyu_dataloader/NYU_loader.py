...
FOLDER_ROOT = "content/NYUv2" 

def get_dataloaders(batch_size=16, output_image_width = 512, output_image_height = 512, custom_transforms=None, shuffle=False) -> Tuple[DataLoader, DataLoader]:
    """
    get_dataloaders returns the train and test dataloaders for the NYUv2 dataset.   
    """
    
    if custom_transforms is None:
        t = transforms.Compose([transforms.Resize([output_image_width,output_image_height]), transforms.ToTensor()])
    else:
        t = custom_transforms

    train = NYUv2(root=FOLDER_ROOT, train=True, download=True)
    test = NYUv2(root=FOLDER_ROOT, train=False, download=True)
...
class NYUv2(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        seg_resize = transforms.Resize(self.image_size, interpolation= transforms.InterpolationMode.NEAREST)
        for i, img in enumerate(imgs):
            
            if i == 1 and self.load_rgb and self.load_depth:
                imgs[i] = torch.round(seg_resize(img))
            elif i == 0 and not self.load_rgb:
                imgs[i] = torch.round(seg_resize(img))
            else:
                imgs[i] = resize(img)

        # if self.random_crop:
        #     i, j, h, w = transforms.RandomCrop.get_params(imgs[0], output_size=self.image_size[0]/2)
        #     for i, img in enumerate(imgs):
        #         imgs[i] = TF.crop(img, i, j, h, w)
        ...
        #     for i, img in enumerate(imgs):
        #         imgs[i] = self.transform_pipeline(img)
